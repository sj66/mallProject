...
Page({
    ...
    app.axios(app.globalData.apiBaseUrl+'index_goods_list.php', {last_id:last_id},'POST').then(res => {
      ...
      if(res_data.status==1){
        ...
        list.forEach((item)=>{
          ...
          if(new Date(item.promote.start_time.replace(/\-/g, "/")).getTime()-new Date().getTime()>0){
            item.promote.end_time=new Date(item.promote.end_time.replace(/\-/g, "/")).getTime()-new Date(item.promote.start_time.replace(/\-/g, "/")).getTime();
          }else{
  ...
  showPopup(e){
    ...
    console.log('商品信息',that.data.list,e.currentTarget.dataset.index,that)
    this.setData({
      order:data[index],
      show:true,
      car_num:1
  ...
  goBuy() {
    console.log('查看id',that.data.order,that)
    if(!that.data.car_num){
      return;
      // that.data.car_num=1;
    }
