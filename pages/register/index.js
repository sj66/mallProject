...
const app = getApp(),phone_reg=/^[1][3,4,5,6,7,8,9][0-9]{9}$/,psd_reg=/^[a-zA-Z0-9]{6,20}$/;
let that,sms_status=false,time_countdown=null;
Page({
  ...
  sendSms(){
    ...
    app. axios(app.globalData.apiBaseUrl+'register_sendSms.php', {
      phone: phone
    },'POST').then(response => {
      wx.hideLoading();
  ...
  onNo(){
    ...
    app. axios(app.globalData.apiBaseUrl+'register.php', {
      userName: phone,
      userPass: psd,
      smsCode: sms_code
    },'POST').then(response => {
      ...
      if(res.status===1){
        ...
        }catch(e){var phone=this.data.account.trim();
          if(phone.length>0){
            if(!phone_reg.test(phone)){   
              app.dialog.tip({msg:'请输入正确的11位手机号码'});
              return false;
